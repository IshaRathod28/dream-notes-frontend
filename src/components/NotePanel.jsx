...
import { getNoteHistory } from "../services/noteHistoryService";
...
// Converts consecutive spaces in HTML text nodes to space+&nbsp; pairs
// so multiple spaces render correctly without CSS white-space side effects.
const preserveSpaces = (html) => {
  if (!html || !/ {2}/.test(html)) return html;
  const el = document.createElement("div");
  el.innerHTML = html;
  const walk = (node) => {
    if (node.nodeType === 3 && / {2}/.test(node.textContent)) {
      node.textContent = node.textContent.replace(/ {2,}/g, (spaces) =>
        spaces.split("").map((s, i) => (i % 2 ? "\u00A0" : s)).join("")
      );
    }
...
function NotePanel({ notebook, resetSignal }) {
  ...
  const openHistory = async (noteId) => {
    ...
    setHistoryItems([]);
    try {
      const data = await getNoteHistory(notebook.id, noteId);
  ...
  return (
    <div ref={panelRef} className="flex flex-col h-full overflow-hidden bg-white dark:bg-gray-950">
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              {viewingNote && !editingNote && (
                <>
                  ...
                  <button
                    onClick={() => openHistory(viewingNote.id)}
                    className="flex items-center gap-1.5 text-sm bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-200 px-4 py-1.5 rounded-lg transition"
                  >
                    🕐 History
                  </button>
                </>
                    ...
                    if (block.type === "text") {
                      ...
                      return (
                        <div
                          ...
                          dangerouslySetInnerHTML={{ __html: preserveSpaces(toEditorContent(block.value)) }}
                        />
                      );
                    }
      ...
      {showHistory && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-6 w-full max-w-md mx-4 border border-gray-200 dark:border-gray-700 max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between mb-4 shrink-0">
              <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100">Note History</h3>
              <button
            ...
            <div className="overflow-y-auto flex-1">
              {historyLoading ? (
                ...
                <div className="text-center py-8 text-gray-400 dark:text-gray-500 text-sm">No history found.</div>
              ) : (
                <div className="flex flex-col gap-3">
                  {historyItems.map((item) => (
                    <div key={item.id} className="flex items-start gap-3 pb-3 border-b border-gray-100 dark:border-gray-800 last:border-0">
                      <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center shrink-0 text-xs font-bold text-blue-600 dark:text-blue-400 uppercase">
                        {(item.user_name || "?").charAt(0)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <span className="text-sm font-semibold text-gray-800 dark:text-gray-100">{item.user_name}</span>
